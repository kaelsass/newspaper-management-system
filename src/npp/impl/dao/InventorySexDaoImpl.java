...
//@Stateless
public class InventorySexDaoImpl implements InventorySexDao {
	...
	@Inject
	public InventorySexDaoImpl(IssueDao issueDao, SubscriberDao subscriberDao, NonSubscriberDao nonSubscriberDao, RetailDao retailDao,
	...
	private List<SubscriberDto> getSubscribers(Transaction transaction,
			InventorySexCondition condition, IssueDto dto) throws IOException {
	...
	private DynamicQuery initQuery(InventorySexCondition condition) {
		DynamicQuery dq = new DynamicQuery();

		dq.addParameter(new Parameter("sex", "=", condition.getSex()));

		return dq;
	}

	private List<NonSubscriberDto> getNonSubscribers(
			Transaction transaction, InventorySexCondition condition, IssueDto dto) throws IOException {
		DynamicQuery dq = initQuery(condition);
		if(dto.getSeq() > 0)
		{
	...
	private List<AdDto> getAds(Transaction transaction, InventorySexCondition condition, IssueDto dto) throws IOException {
		DynamicQuery dq = new DynamicQuery();
