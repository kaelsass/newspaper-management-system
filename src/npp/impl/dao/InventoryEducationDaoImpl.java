...
//@Stateless
public class InventoryEducationDaoImpl implements InventoryEducationDao {
	...
	@Inject
	public InventoryEducationDaoImpl(IssueDao issueDao, SubscriberDao subscriberDao, NonSubscriberDao nonSubscriberDao, RetailDao retailDao,
	...
	private List<SubscriberDto> getSubscribers(Transaction transaction,
			InventoryEducationCondition condition, IssueDto dto) throws IOException {
	...
	private DynamicQuery initQuery(InventoryEducationCondition condition) {
		DynamicQuery dq = new DynamicQuery();

		dq.addParameter(new Parameter("education_seq", "=", condition.getEducationSeq()));
	...
	private List<NonSubscriberDto> getNonSubscribers(
			Transaction transaction, InventoryEducationCondition condition, IssueDto dto) throws IOException {
		DynamicQuery dq = initQuery(condition);
		if(dto.getSeq() > 0)
		{
	...
	private List<AdDto> getAds(Transaction transaction, InventoryEducationCondition condition, IssueDto dto) throws IOException {
		DynamicQuery dq = new DynamicQuery();
