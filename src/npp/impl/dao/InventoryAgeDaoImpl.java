...
//@Stateless
public class InventoryAgeDaoImpl implements InventoryAgeDao {
	...
	@Inject
	public InventoryAgeDaoImpl(IssueDao issueDao, SubscriberDao subscriberDao, NonSubscriberDao nonSubscriberDao, RetailDao retailDao,
	...
	private List<SubscriberDto> getSubscribers(Transaction transaction,
			InventoryAgeCondition condition, IssueDto dto) throws IOException {
	...
	private DynamicQuery initQuery(InventoryAgeCondition condition) {
		DynamicQuery dq = new DynamicQuery();
		if(condition.getAgePair().getSeq() >= 0)
		{
			dq.addParameter(new Parameter("age", ">=", condition.getAgePair().getFrom()));
			dq.addParameter(new Parameter("age", "<=", condition.getAgePair().getTo()));
		}
		else
		{
			dq.addParameter(new Parameter("age", "=", condition.getAgePair().getFrom()));
	...
	private List<NonSubscriberDto> getNonSubscribers(
			Transaction transaction, InventoryAgeCondition condition, IssueDto dto) throws IOException {
		DynamicQuery dq = initQuery(condition);
		if(dto.getSeq() > 0)
		{
	...
	private List<AdDto> getAds(Transaction transaction, InventoryAgeCondition condition, IssueDto dto) throws IOException {
		DynamicQuery dq = new DynamicQuery();
