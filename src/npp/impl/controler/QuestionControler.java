...
import npp.spec.service.QuestionService;
...
@Named
@SessionScoped
public class QuestionControler implements Serializable {
	...
	public StatusListModel getListModel() {
		if (baseList == null) {
			try {
				baseList = questionService.getAllList();
	...
	public void apply() throws IOException {
		if (addMode)
			questionService.add(editDto);
		else if (editMode)
			questionService.update(editDto);
		clear();
	}
	...
	public void delete() throws IOException {
		try {
			for (StatusDto status : selecteds) {
				questionService.delete(status.getSeq());
			}
			clear();
		} catch (IOException e) {
